...
model_champagne2022_seasonal_poisson = "champagne2022_seasonal_poisson.stan"
...
poisson_seasonal_sol = function(.cases, .population_size, .alpha, .beta, .omega, .eps) {
  .data = data_consts
  .data$cases = .cases
  .data$population_size = .population_size
  .data$alpha = .alpha
...
for (i in seq_len(nrow(data_scenarios))) {
  lambda_nonseasonal_poisson[[i]] = with(data_scenarios, {
    poisson_nonseasonal_sol(cases[[i]], population_size[i], ascertainment_rates[i], radical_cure_rates[i], 1)
  })
}
...
for (i in seq_len(nrow(data_scenarios))) {
  lambda_nonseasonal[[i]] = with(data_scenarios, {
    nonseasonal_sol(cases[[i]], population_size[i], ascertainment_rates[i], radical_cure_rates[i], 1)
  })
}
...
for (i in seq_len(nrow(data_scenarios))) {
  lambda_seasonal_poisson[[i]] = with(data_scenarios, {
    poisson_seasonal_sol(cases[[i]], population_size[i], ascertainment_rates[i], radical_cure_rates[i], 1, seasonality_ratio[i])
  })
...
for (i in seq_len(nrow(data_scenarios))) {
  lambda_seasonal[[i]] = with(data_scenarios, {
    seasonal_sol(cases[[i]], population_size[i], ascertainment_rates[i], radical_cure_rates[i], 1, seasonality_ratio[i])
  })
  # print(data_scenarios$lambda_nonseasonal[i])
}
...
plot_data %>%
  ...
  ggplot(aes(x = transmission_rates, y = est_mean, color=seasonality_ratio)) +
  geom_point() +
...
plot_data = plot_data %>%
  group_by(ID, method) %>%
  mutate(lq = unname(quantile(estimate, trim*0.5)),
...
for (id in unique(plot_data$ID)) {
  ...
  .plot_posterior = plot_data %>%
    filter(ID == id) %>%
    group_by(ID, method) %>%
    mutate(lq = unname(quantile(estimate, 0.025, na.rm=T)),
           uq = unname(quantile(estimate, 0.975, na.rm=T))) %>%
    filter(estimate > lq,
           estimate < uq) %>%
    filter(n() > 1) %>%
    ungroup() %>%
    select(-ID, -ascertainment_rates, -radical_cure_rates, -population_size, -cases, -ts)# %>%
    # filter(method == "lambda_seasonal_poisson")
  
  title = with(.plot_posterior[1,], {
    paste0(id, ": lambda=", transmission_rates, " eps=", seasonality_ratio)
  })
  
  p1 = ggplot(.plot_cases, aes(x = ts, y = cases)) +
    geom_line() +
    labs(title = title, x = "Days")
  
  p2 = ggplot(.plot_posterior, aes(x = estimate, y = ..scaled.., fill = method, color = method)) +
    stat_density(position="identity", alpha = 0.3, bw=0.0001) +
  ...
  p3 = ggplot(.plot_posterior %>% group_by(method) %>% mutate(trace = row_number()), aes(x = trace, y = estimate,, color = method)) +
    geom_line(alpha = 0.5) +
    # geom_histogram(binwidth = 0.001, alpha = 0.5) +
