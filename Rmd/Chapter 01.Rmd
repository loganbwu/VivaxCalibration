...
```{r}
params_extract_1 = rstan::extract(fit, c("lambda", "phi_inv")) %>%
  lapply(as.numeric) %>%
  as_tibble()
real_params_df = as.data.frame(real_params) %>%
  pivot_longer(everything())
...
radical_cure_rates = c(0.25, 0.5, 0.75, 1)
ascertainment_rates = data_consts$alpha
# radical_cure_rates = data_consts$beta
transmission_rates = c(0.005, 0.01, 0.02)
...
1. Try Champagne's direct solution
...
#          h = sapply(cases, mean) * 30.4 / population_size,
...
mydata = data_scenarios %>%
  mutate(id = LETTERS[row_number()],
         h = sapply(cases, mean) / 30.4 / population_size, # daily incidence per person
         alpha = 0.9,
         beta = 0.9,
         rho = 0.9,
         omega = 1,
...
2. Aggregate monthly data to annual data then use the non-seasonal model

```{r}
data = data_consts
data$n_times = floor(n_times / annual_subdivisions)
ts = seq_len(n_times) * dt * annual_subdivisions
every_twelfth = seq(1, length(synth_df$ts), annual_subdivisions) # c(1, 13, 25, ...)
data$ts = synth_df$ts[every_twelfth]
data$cases = sapply(every_twelfth, function(mo) {
  ixs = seq_along(synth_df$cases)
  sum(synth_df$cases[ixs >= mo & ixs < (mo+annual_subdivisions)])
...
pairs(fit, pars=c("lambda", "phi_inv"))
params_extract_2 = rstan::extract(fit, c("lambda", "phi_inv")) %>%
  lapply(as.numeric) %>%
  as_tibble()
real_params_df = as.data.frame(real_params) %>%
  pivot_longer(everything())
...
sim_scenarios = lapply(seq_len(nrow(data_scenarios)), fun)
...
analysis %>%
  filter(name %in% c("lambda", "phi_inv")) %>%
  ggplot(aes(x = ascertainment_rates, y = transmission_rates, fill=mean, color=mean)) +
  geom_point() +
