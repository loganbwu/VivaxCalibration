...
ggplot(rondonia.all, aes(x = notification.date, y=cases, fill=include)) +
  annotate("rect", xmin=as.Date("min_year-01-01"), xmax=as.Date("max_year-12-31"), ymin=-Inf, ymax=Inf,
...
rondonia.nonstationary = integrated_dataset %>%
  ...
  count(notification.year, notification.date, name = "cases") %>%
  mutate(include = ifelse(notification.year >= 2009 & notification.year <= max_year, "grey20", "grey")) %>%
  as_tibble()

p1 = ggplot(rondonia.nonstationary, aes(x = notification.date, y=cases, fill=include)) +
  annotate("rect", xmin=as.Date("2009-01-01"), xmax=as.Date("max_year-12-31"), ymin=-Inf, ymax=Inf,
           fill = "blue", alpha = 0.15) +
  geom_col() +
  coord_cartesian(ylim = c(0, NA)) +
  scale_x_date(date_breaks = "years", date_labels = "%Y") +
