...
(p_seasonal_posterior + theme(legend.position="none") + labs(title = NULL, subtitle = "Seasonal model")) + p_ns_posterior + theme(legend.position="none") +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Posterior parameter estimates at equilibrium",
                  subtitle = paste0("P. vivax transmission in Rondonia, ", min_year, "-", max_year),
                  tag_levels = "A")
...
rondonia.all %>%
  mutate(include = ifelse(notification.year <= max_year, "grey20", "grey")) %>%
  ggplot(aes(x = notification.date, y=cases, fill=include)) +
  annotate("rect", xmin=as.Date(min(rondonia_ns$notification.date)), xmax=as.Date(paste0(max_year, "-12-31")), ymin=-Inf, ymax=Inf,
           fill = "blue", alpha = 0.15) +
  geom_col() +
  coord_cartesian(ylim = c(0, NA)) +
  scale_x_date(date_breaks = "years", date_labels = "%Y") +
...
ggsave("../plots/rondonia_nonstationary_selected.png", width=8, height=2)
```
...
ggplot(mapping = aes(x=ts / years + 2009)) +
  ...
  geom_point(data = tibble(ts = data_ro_ns$ts, cases = data_ro_ns$cases),
             aes(y = cases, group = NULL, color="Case data")) +
  coord_cartesian(ylim = c(0, NA)) +
  scale_x_continuous(breaks = 2019:2019) +
  scale_fill_manual("", 
                    breaks = c("95% case prediction interval", "95% mean incidence interval", "Case data"),
                    values = c("steelblue", "tomato", "black")) +
  scale_colour_manual("", 
...
posterior_comparison = bind_rows(Stationary = posterior_seasonal,
  ...
  filter(!parameter %in% c("tstar", "xi"))
posterior_ranges_2 =  posterior_ranges %>%
  mutate(Model = NA, parameter = ifelse(parameter == "lambda", "λ(t>t*)", parameter)) %>%
  filter(!parameter %in% c("tstar", "xi"))
ggplot(posterior_comparison, aes(x = value, fill = Model, color=Model)) +
  geom_blank(data = posterior_ranges_2) +
  geom_density(alpha = 0.5) +
