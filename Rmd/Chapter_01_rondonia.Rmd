...
every_twelfth = seq(1, length(data_ro_agg$cases), annual_subdivisions)
data_ro_agg$ts = data_ro_agg$ts[every_twelfth]
data_ro_agg$cases = sapply(every_twelfth, function(mo) {
  ixs = seq_along(data_ro_agg$cases)
  sum(data_ro_agg$cases[ixs >= mo & ixs < (mo+annual_subdivisions)])
})
data_ro_agg$eps = 1
```
