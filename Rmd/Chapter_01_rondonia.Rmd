...
synth_data = simulate_data(
  ...
  vars = c("ts", "incidence", "sim_cases", "foi", "susceptible", "infectious", "dormant")
)
...
synth_df %>%
  pivot_longer(c(susceptible, infectious, dormant), names_to="compartment", values_to="proportion") %>%
...
composite_list = pbmclapply(seq_len(n_samples), function(i) {
  ...
  composite_data_rds = readRDS(composite_data$datasets[1])
  file.remove(composite_data$datasets[1])
  indx <- sapply(composite_data_rds, length)
  lapply(composite_data_rds, function(x) {length(x) = max(indx); x}) %>%
    as.data.frame() %>%
    drop_na()
})
