...
```{r}
posterior = rstan::extract(samp_ns, c("lambda", "phi_inv", "eps", "kappa", "phase", "tstar", "xi")) %>%
  as_tibble() %>%
  mutate(phi = 1 / phi_inv,
         tstar = tstar / years + 2010) %>%
  select(-phi_inv)

posterior_ranges = tribble(
  ~parameter, ~min, ~max,
  "lambda", 0, max(posterior$lambda * 1.5),
  "phi", 0, max(posterior$phi * 1.5),
  "eps", 0, 1,
  "kappa", 0, max(posterior$kappa * 1.5),
  "phase", 0, years,
  "tstar", 2010, 2020,
  "xi", 0, 1
) %>%
...
ggplot(mapping = aes(x=ts / years + 2010)) +
  geom_ribbon(data = incidence_sample_nonseasonal,
              aes(ymin=lower, ymax=upper, fill="95% mean incidence interval"),
              alpha = 0.5) +
