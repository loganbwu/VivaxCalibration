...
model_file = "../stan/temperate_4.stan"
...
data = data_init(data_null,
                 ...
                 gamma_l = 0,
                 gamma_d = 0)
data$y0 = state_init_v4(data, list(1) %>% setNames(paste0("Sl", data$n_dormant+1)))

true_params = list(lambda = 0.0)

sim_out = sampling(model, data = data, chains = 1, iter = 1)
sim_summary = extract_summary_v4(data, sim_out)
...
data = data_init(data_null,
                 ...
                 gamma_d = 0,
                 f = 99)
data$y0 = state_init_v4(data, Sl1=1)
...
plot_data = sim_summary %>%
  ...
  mutate(Type = ifelse(Compartment == "ClinicalIncidence", "Incidence", "Compartment"))
...
data$y0 = state_init_v4(data, "Icl1" = 1)
real_params = list(lambda = 1e-12)
...
data$y0 = state_init_v4(data, "I0" = 0.01)
...
data_1$y0 = state_init_v4(data, Sl12 = 1)
data_2$y0 = state_init_v4(data, Sl1 = 0.5, Sl12 = 0.5)
data_3$y0 = state_init_v4(data, Sl1 = 1)
real_params = list(lambda = 1e-12)
...
plot_data = sims_out %>% bind_rows(.id = "Var") %>%
  pivot_longer(c(ClinicalIncidence, Infectious, Dormant, Latent, Susceptible)) %>%
  mutate(Type = ifelse(name %>% str_starts("Reported|Clinical"), "Incidence", "Compartment"))
...
sims_out = list(data_1, data_2, data_3) %>%
  ...
  lapply(function(x) {
    x$y0 = state_init_v4(x, I0 = init_I0)
    my_simulate_data_list(
      file = model_file,
      data_name = "dummy_data",
      input_data = x,
...
data = data_init(data_null,
                 ...
                 gamma_l = 0,
                 gamma_d = 0)
data$y0 = state_init_v4(data, I0 = 0.1)
...
plot_data = sim_out %>%
  pivot_longer(c(ClinicalIncidence, Infectious, Dormant, Latent, Susceptible, omega)) %>%
  mutate(Type = ifelse(name =="ClinicalIncidence", "Incidence", "Compartment"))
