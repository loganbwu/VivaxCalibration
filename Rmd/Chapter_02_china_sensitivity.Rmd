...
model = stan_model("../stan/temperate_11_2_plausible.stan")
...
for (data_name in names(data_experiments)) {
  ...
  samp = sampling(model,
                  ...
                  include = F)
  tictoc::toc()
  samp_results_sens[[data_name]] = samp
}
```
...
posterior_seasonal = lapply(samp_results_sens, function(samp) {
...
traces = lapply(samp_results_sens, function(samp) {
  ...
  bind_rows(`Post. pred. cases` = cases_sample_temperate,
    ...
    group_by(j, metric) %>%
    summarise(ts = data_experiments[[1]]$ts[j],
