...
ggplot(plot_data_2, aes(x = value, y = 1, color = method, fill = method, group = interaction(lambda, rep, method))) +
...
```{r}
scenarios_long$est_incidence[[10]] %>%
  ggplot(aes(x=ts/years, y=incidence, color=as.factor(rep), group=interaction(trace, rep))) +
...
resimulated = lapply(seq_len(nrow(valid_scenarios_long)), function(i) {
  ...
  #   return(NULL)
  # }
  pblapply(seq_len(n_traces), function(j) {
...
ggplot(resimulated_2, aes(x = ts/years, y = incidence, color=method, group = interaction(method, trace))) +
  ...
  labs(title = "Re-simulated incidence rates for the first scenario repetition",
       subtitle = "Incidence of very near zero (elimination) not plotted on log scale",
       x = "Year",
       y = "Incidence", color = "Method") +
  guides(color = guide_legend(override.aes = list(alpha = 1)))
