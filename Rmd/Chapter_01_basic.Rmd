...
incidence_seasonal = extract_incidence(fit_seasonal, data_sim)
...
incidence_nonseasonal = extract_incidence(fit_nonseasonal, data_sim_agg)
...
fits_scenarios = pbmclapply(seq_len(nrow(scenarios_long)), function(i) {
  my_sampling_indexed(scenarios_long$method[i], scenarios_long$ts[[i]], scenarios_long$cases[[i]])
})

# Extract parameters into dataframe
estimates_scenarios = lapply(fits_scenarios, function(x) {
  posterior = rstan::extract(x, c("lambda", "phi_inv", "eps", "kappa", "phase")) %>%
    as_tibble()
})

# Attach re-estimated parameters
scenarios_long$est_lambda = lapply(estimates_scenarios, function(x) {x$lambda})
scenarios_long$est_phi_inv = lapply(estimates_scenarios, function(x) {x$phi_inv})
...
scenarios_long$est_incidence = lapply(seq_len(nrow(scenarios_long)), function(x) {
  ...
  scenario = scenarios_long[i,]
  method = scenario$method
  data_sim = data_init(data_consts, n_times = length(scenario$ts[[i]]), ts = scenario$ts[[i]], cases = scenario$cases[[i]])
...
for (param in c("lambda", "phi_inv", "eps", "kappa", "phase")) {
