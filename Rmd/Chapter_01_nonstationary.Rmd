...
data = data_init(data_consts,
                 ...
                 cases = rep(1, n_times),
                 n_times = n_times)
params = list(lambda=0.01, phi_inv=0.1, eps=0.01, kappa=1, phase=0, xi=0.9, tstar=tstar_for_testing)
...
```{r}
posterior = rstan::extract(fit_tstar, c("lambda", "phi_inv", "eps", "kappa", "tstar", "xi")) %>%
  as_tibble()
...
for (param in c("lambda", "tstar", "xi")) {
  p = scenarios %>%
    ...
    ggplot(aes(x = value, fill = param, color = param)) +
    geom_density(alpha = 0.5) +
    geom_vline(xintercept = 0.01) +
    facet_grid(rows = vars(tstar), cols = vars(xi), labeller = plot_labeller, scales="free") +
    scale_fill_manual(values = param_colors, drop=F) +
    scale_color_manual(values = param_colors, drop=F) +
