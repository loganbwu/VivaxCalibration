...
```{r}
simulate_cases = function(data_consts, lambda, phi_inv, eps_kappa, phase, xi, tstar) {
  n_times = annual_subdivisions * n_years
  data = data_init(data_consts,
                   cases = rep(1, n_times),
                   ts = seq_len(n_times) * years / annual_subdivisions,
...
cases_scenarios = pbmclapply(seq_len(nrow(scenarios)), function(i) {
  ...
  xi = scenarios$xi[i]
  simulate_cases(data_consts, lambda, phi_inv, eps_kappa, phase, xi, tstar)
})
...
scenarios %>%
  ...
  labs(title = "Synthetic scenario data",
       x = "Year",
       y = "Monthly cases")
```
...
for (param in c("lambda", "eps", "kappa", "phase", "tstar", "xi")) {
  plot_data = scenarios %>%
...
```{r}
incidence_scenarios = lapply(fits_scenarios, extract_incidence)
...
ggplot(incidence_join, aes(x = ts, y = incidence, color=factor(rep), group = rep)) +
  ...
  # geom_vline(data=true_values, aes(xintercept = true_value), linetype="dashed") +
  # geom_density(alpha = 0.5) +
  facet_grid(rows = vars(tstar), cols = vars(xi), labeller = plot_labeller) +
  # scale_fill_manual(values = param_colors, drop=F) +
  # scale_color_manual(values = param_colors, drop=F) +
