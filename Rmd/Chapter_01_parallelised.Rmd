...
.simulate_cases = function(alpha=0.5, beta=0.5, lambda=1, phi_inv=0.1, delta=0, eps=0, kappa=1, phase=0, N=100, index=NULL) {
  data = data_consts
  data$alpha = alpha
  data$beta = beta
  data$delta = delta
  ...
  real_params = list(lambda=lambda, phi_inv=phi_inv, eps=eps, kappa=kappa)
  print(real_params)
...
brazil_result = list(fit = fit)

posterior_lambda = extract_lambda(brazil_result)
posterior_ = extract_lambda(brazil_result)
```
