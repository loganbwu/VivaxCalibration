...
# radical_cure_rates = data_consts$beta
transmission_rates = seq(0.03, 0.09, length.out=3)
...
# for (i in seq_len(nrow(data_scenarios_long))) {
#   print(i)
#   fits_all[[i]] = run_scenario_method(i)
# }
...
for (i in seq_len(nrow(data_scenarios_long))) {
  ...
  if (result@mode == 2) {
    ...
    fits_all[[i]] = run_scenario_method(i, force=T)
  }
}
tictoc::toc()
...
plot_data = lapply(valid_ix, function(i) {
  ...
  select(scenario_ID, seasonality_ratio, transmission_rates, ts, method, rep, daily_incidence, trace)
...
ggplot(plot_data, aes(x=ts, y=daily_incidence, color=method, group=interaction(trace, method))) +
  geom_line(alpha=0.2) +
  geom_point(data=data_scenarios_daily, aes(x=ts, y=daily_cases, color=NULL, group=NULL), alpha=0.25) +
