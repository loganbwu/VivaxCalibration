...
for (id in unique(plot_data$ID)) {
  ...
  limits = c(true_value * 0.75, true_value * 1.25)
...
message(sum(not_run), " of ", nrow(data_scenarios_long), " did not complete sampling")
data_scenarios_long$time[not_run] = -10
data_scenarios_long %>%
  mutate(time = unlist(time)) %>%
  ggplot(aes(x = time, fill = method)) +
  geom_histogram() +
  labs(title = "Scenario runtime",
       subtitle = "Scenarios terminated after timeout are placed at -10")
```
