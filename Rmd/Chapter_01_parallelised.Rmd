...
# radical_cure_rates = data_consts$beta
transmission_rates = seq(0.0275, 0.03, length.out=3)
...
for (i in seq_len(n_scenarios)) {
  ...
  scenario_cases = data_scenarios_sim %>%
    ...
    unnest(c(ts, cases)) %>%
    select(ts, cases, transmission_rates, seasonality_ratio)
  
  # Case plot
  p1 = ggplot(scenario_cases, aes(x = ts/years, y = cases)) +
    geom_point() +
    labs(x = "Years",
         y = "Cases")
...
rhat_data %>%
  ...
  labs(title = "Did the runs converge?",
       subtitle = paste0(nrow(nonconvergence), " runs had an rhat on lambda greater than ", rhat_threshold, " (max ", max(round(rhat_data$rhat, 2)), ")"),
       fill = "Method")
```
...
ggplot(modelled_incidence, aes(x=ts, y=incidence, color=method, group=interaction(rep, trace, method))) +
  ...
  labs(title = "Posterior mean incidence trajectories and overlaid case data",
       color = "Method",
       x = "Time (years)",
       y = "Incidence")
```
...
composite = lapply(seq_len(nrow(data_scenarios_long)), function(i) {
  df = data_scenarios_long[i,] %>%
    select(-cases)
...
ggplot(composite, aes(x=ts, y=incidence, color=method, group=interaction(method, sim))) +
  geom_line(alpha=0.2) +
  geom_point(data=plot_cases, aes(x=ts, y=cases, color=NULL, group=NULL), alpha=0.25) +
  facet_grid(rows = vars(seasonality_ratio),
             cols = vars(transmission_rates), scales="free",
