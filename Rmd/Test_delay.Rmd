...
data$kappa = 1
data$phase = 0
# data$ts = seq(0, 365.25*10, length.out = 1000)
# data$n_times = length(data$ts)
# data$cases = rpois(data$n_times, 100)
data$y0 = state_init(data, I0 = 0.01)
real_params = list(lambda = 10)
...
sim_out %>%
  pivot_longer(-time) %>%
  filter(name %in% names(data$y0),
         !name %>% str_detect("True")) %>%
  ggplot(aes(x = time, y = value, color = name)) +
  geom_line()
