...
# load("~/Downloads/Chapter_02_china_metropolis_latest.RData")
...
If using a saved workspace file (e.g. from the HPC), don't run the below chunks. Skip to the section 'Load simulations from HPC'.
...
if (!require(MalariaData)) {
  # Because the data package is not guaranteed to be installed
  message("Loading minimal data packaged with repository")
...
Now we define parameters for scenarios to run the model on
...
Define the initial values and a list to put results in
...
# Define a function to initialise a scenario
stored_model_stats_file = "model_stats.rds" # exists to initialise repeated runs from a previous run's samples
