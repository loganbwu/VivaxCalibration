...
posterior_seasonal = lapply(samp_results, function(samp) {
  ...
  mutate(parameter_name = parameter_name %>% str_replace_all(", ", ",\n"))
...
ll_plot = posterior_seasonal %>%
  ggplot(aes(x = ll, y = parameter_name, color=parameter_name, fill=parameter_name, group=interaction(parameter_name, chain))) +
...
epi_plot = ggplot(mapping = aes(x=time/years)) +
  ...
  facet_grid(rows=vars(parameter_name))
  labs(x = "Year",
       y = "Monthly incidence") +
