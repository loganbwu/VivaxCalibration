...
# samp_results = readRDS("samp_results.rds")
```
...
```{r}
for (i in seq_along(samp_results)) {
  if ("transformed" %in% names(samp_results[[i]])) {
    next
  }
  sim = samp_results[[i]]$sim
  new_sim = lapply(sim, function(x) {
    x %>%
      mutate(s = 1 - p_long,
             p = 1 - p_silent,
             p_long = NULL,
  ...
  samp_results[[i]]$sim = new_sim
  
  ess = samp_results[[i]]$ESS
  new_ess = lapply(ess, function(x) {
    names_ess = names(x)
    names_ess[names_ess=="p_long"] = "s"
    names_ess[names_ess=="p_silent"] = "p"
    x %>% setNames(names_ess)
  })
  samp_results[[i]]$ESS = new_ess
  
  current_x = samp_results[[i]]$current_x
  new_current_x = lapply(current_x, function(x) {
    names_current_x = names(x)
    names_current_x[names_current_x=="p_long"] = "s"
    names_current_x[names_current_x=="p_silent"] = "p"
    x %>% setNames(names_current_x)
  })
  samp_results[[i]]$current_x = new_current_x
  
  samp_results[[i]]$transformed = TRUE
}
```
...
for (i in seq_len(nrow(var_ranges))) {
  ...
  # insert range to evaluate prior over
    ...
    if (parameter_name == "s") {
      x_dummy["p_long"] = 1-x
    } else if (parameter_name == "p") {
      x_dummy["p_silent"] = 1-x
    } else {
...
ess_plot = ggplot(ess_df, aes(x = ess, y = parameter, fill = region)) +
  ...
  # theme(legend.position = "none") +
  labs(subtitle = "Effective sample size (higher is better)",
       x = "ESS",
       y = "Parameter",
       fill = NULL) +
  theme(axis.text.y = element_markdown())

rhat = pblapply(samp_results, rhat.metropolisfit)
