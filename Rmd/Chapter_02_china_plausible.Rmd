...
for (selected_region in selected_regions) {
  ...
  samp = sampling(model,
                  ...
                  include = F)
  tictoc::toc()
  samp_results_plausible[[selected_region]] = samp
}
```

```{r}
# inspect parameter posteriors
parameters = c("lambda", "phi", "alpha", "beta", "eps", "kappa", "phase", "psi", "relapse_clinical_immunity", "p_long", "p_silent")

posterior_seasonal = lapply(samp_results_plausible, function(samp) {
...
traces = lapply(samp_results_plausible, function(samp) {
  ...
  bind_rows(incidence = incidence_sample_temperate,
            cases = cases_sample_temperate,
            .id = "metric") %>%
    group_by(j, metric) %>%
...
ggplot(mapping = aes(x=ts/years)) +
  ...
  scale_fill_manual("", 
                    breaks = c("cases", "incidence", "Incidence data"),
                    values = c("steelblue", "tomato", "black")) +
  scale_colour_manual("", 
                      breaks = c("cases", "incidence", "Case observations"),
                      values = c("steelblue", "tomato", "black")) +
...
workspace_filename = paste0("workspaces/Chapter_02_china_plausible", Sys.Date(), ".RData")
